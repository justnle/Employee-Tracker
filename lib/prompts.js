...
function getDatabaseInfo(table, column) {
  if (table === `employee` && column === `full_name`) {
    app.connection.query(queries.employeesFullName, (err, res) => {
      ...
      fillArrays(employeeChoices, employees.full_name);
    });
  } else {
    app.connection.query(`SELECT ${column} FROM ${table}`, (err, res) => {
      ...
      switch (column) {
      case `name`:
        fillArrays(departmentChoices, department.name);
        break;
      case `title`:
        fillArrays(roleChoices, role.title);
        break;
      case `id`:
        if (table === `employee`) {
          fillArrays(employeeIds, id.id);
        } else if (table === `department`) {
          fillArrays(departmentIds, id.id);
        } else if (table === `role`) {
          fillArrays(roleIds, id.id);
        }
        break;
      default:
...
function fillArrays(arr, property) {
  arr.length = 0;
