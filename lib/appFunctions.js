...
function viewEmployees() {
  db.connection.query(queries.allEmployees, (err, res) => {
    if (err) throw err;
...
function view(choice) {
  switch (choice) {
    case 'departments':
      db.connection.query(queries.allDepartments, (err, res) => {
        if (err) throw err;
      ...
      break;
    case 'roles':
      db.connection.query(queries.allRoles, (err, res) => {
        if (err) throw err;
...
function getDatabaseInfo(table, column) {
  ...
  if (table === 'employee' && column === 'full_name') {
    ...
    db.connection.query(queries.employeesFullName, (err, res) => {
      if (err) throw err;
    ...
    db.connection.query(`SELECT ${column} FROM ${table}`, (err, res) => {
      if (err) throw err;
