  ...
  switch (choice) {
    case 'employeeRole':
      prompt(prompts.updateEmployeeRole).then(answer => {
        ...
        let employeeId = employeeIds[employeeIdIndex];
        let newRoleIndex = roleChoices.indexOf(answer.roleUpdate);
        let newRole = roleIds[newRoleIndex];
    ...
    case 'employeeManager':
      prompt(prompts.updateEmployeeManager).then(answer => {
        let employeeIdIndex = employeeChoices.indexOf(answer.updateEmployee);
        let employeeId = employeeIds[employeeIdIndex];
        let newManagerIndex = employeeChoices.indexOf(answer.managerUpdate);
        let newManager = employeeIds[newManagerIndex];
