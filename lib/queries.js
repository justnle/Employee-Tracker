...
const employeesWhere = `SELECT concat(first_name, ' ', last_name) AS 'Full Name' FROM employee WHERE ?`;

const allManagers = `
SELECT concat(employee.first_name, ' ', employee.last_name) AS Managers, role.title AS Title, department.name AS Department
FROM employee
  INNER JOIN role
    on (employee.role_id = role.id)
  INNER JOIN department
    on (role.department_id = department.id)
  WHERE role.id IN (1, 3, 6)
  ORDER BY employee.id
`;
